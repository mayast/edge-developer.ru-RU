...
-   Старый обработчик Chakra (также называемый *устаревшим модулем* или Jscript9. dll ниже), который поставляется вместе с Internet Explorer 11 и поддерживает его. Этот обработчик зафиксирован в течение времени и останется без изменений в выпуске Win 8.1/IE11.  
  
-   Новый обработчик Chakra (также называемый *ядром пограничного* приложения или Chakra. dll), который поставляется вместе с новым браузером в Windows 10, Microsoft EDGE, и поддерживает его. Этот обработчик будет постоянно обновляться и будет поддерживать обработчик [Edge](https://blogs.msdn.com/b/ie/archive/2014/11/11/living-on-the-edge-our-next-step-in-interoperability.aspx) "живых". Работа работающего подсистемы Microsoft EDGE, в отличие от устаревшего обработчика, не перенесет функцию Microsoft Edge Engine, чтобы принять участие в программе.  
...
-   JsRT для Магазина Windows поддерживает только обработчик Microsoft EDGE (Chakra. dll). Приложения, пытающиеся установить связь с любым API JsRT в Jscript9. dll, не смогут пройти сертификацию.  
