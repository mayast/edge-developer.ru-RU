...
---
# интерфейс ICoreWebView2Controller 
...
В оконном режиме этот обработчик событий вызывается синхронно. До тех пор пока вы не выйдете метод Handled () для аргументов события или не будет возвращен обработчик событий, процесс браузера будет заблокирован и исходящие межпроцессные вызовы COM не будут работать с RPC_E_CANTCALLOUT_ININPUTSYNCCALL. Однако все методы API CoreWebView2 будут работать.

В режиме безоконный обработчик событий вызывается асинхронно. Дальнейшие входные данные не доходят до браузера, пока обработчик событий не возвратит или не обработал (), но сам процесс браузера не будет заблокирован, а исходящие вызовы COM будут работать нормально.

Рекомендуется вызывать метод Handled (TRUE), как только вы можете узнать, что вы хотите обработать клавишу вызова.
...
> общедоступные значения HRESULT [get_ParentWindow](#get_parentwindow)(HWND * ParentWindow)
...
> общедоступные значения HRESULT [put_ParentWindow](#put_parentwindow)(HWND ParentWindow)
